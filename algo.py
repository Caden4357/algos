...
def minMax(lst):
    ...
    if len(lst) < 2:
        ...
        lst.append(min)
        return lst
print(minMax([4,57,8,99,100,54,1,0,1]))

#Create an app to do factorial ex. Factorial algo 4*3*2*1
def fact(num):
    sum = 1
...
#find out if a number is prime which means its only divisible by itself and 1
def isPrime(num):
...
#sort a list from the smallest to the greatest value 
...
def sorting(lst):
    ...
    return lst
print(sorting([8,7,6,4,15,3,1,2]))
